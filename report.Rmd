...
The goodness-of-fit for both the placement model and the competition performance model was assessed with the Hosmerlem-Lemeshow, Lipsitz, and Pulksteins-Robinson tests, which are all recommended model diagnostics for [ordinal regression](https://journals.sagepub.com/doi/pdf/10.1177/1536867X1701700308#:~:text=3%20The%20goodness%2Dof%2Dfit,is%20wrong%20with%20the%20model.). These tests were implemented with the [`gofcat`](https://cran.r-project.org/web/packages/gofcat/gofcat.pdf) library in R; no test showed significant evidence of a lack of fit for either model (see **Appendix**.)
